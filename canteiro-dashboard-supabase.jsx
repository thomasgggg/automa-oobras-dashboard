...
export default function CanteiroDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const saved = await window.storage.get("supabase-config", false);
        if (saved) {
  ...
  async function saveConfig() {
    ...
    try {
      await window.storage.set("supabase-config", JSON.stringify(next), false);
    } catch (e) {}
    setConfig(next);
  ...
  async function loadData() {
    ...
    try {
      ...
      setError("Não consegui conectar no banco. Confira a URL e a chave em Configurações.");
    }
    setSyncing(false);
  }
